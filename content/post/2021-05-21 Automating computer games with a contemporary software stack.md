...
* RGB distance, which compares $\vec{C}_1 \cdot \vec{C}_2$ and is a bit slower, but closer to "perceptually similar" than a simple difference.
* HSL difference, which first converts both colors to the [HSL color space](https://en.wikipedia.org/wiki/HSL_and_HSV) and checks that hue, saturation, and lightness are within some value for the two colors. Much slower, but closer to 'perceptually similar' than the other options.
